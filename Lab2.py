...
#import bottle
from bottle import route, run, template, Bottle, request, static_file, redirect, app
import httplib2

client_id = '247769669257-6fnk6h0apofc39hhehbtnckrr5m0odie.apps.googleusercontent.com'
client_secret = 'wkBH2qk9pykSn2rvxwG_jYgK'
scope = ['profile', 'email']
#ip = 'viniciusdantas.me'
ip = '172.31.62.94'
ip2 = '54.172.103.193'
#redirect_uri = 'http://localhost:8080/redirect'
#redirect_uri = 'http://'+ip2+'/redirect'
redirect_uri = 'http://viniciusdantas.me/redirect'
...
def hello():
    ...
    if not request.forms.get('keywords') is None:
        ...
        return template('index', user=s.get('user',None), words=var_query,number_of_words = len(var_query.split()),results=find_count(var_query),history=find_top_20(find_count(var_query)))
    else:
        return template('index', user=s.get('user',None), words=var_query,number_of_words = len(var_query.split()),results=find_count(var_query),history=find_top_20(find_count(var_query)))
